...
import { DEFAULT_TEMPERATURE, DEFAULT_MAX_TOKENS, AI_TIMEOUT_MS } from './constants.js';
...
export class CouncilOrchestrator {
  ...
  public aiClient: AIClient; // Public for utility access
  private toolSystem: ToolSystem;
  private responseCallback?: (response: ExpertResponse, documentId: string) => Promise<void>;
  ...
  async orchestrate(
    ...
    const promises = experts.map(expert =>
      this.executeExpert(documentId, userMessage, triggerUserId, expert, context, isIndirectInvocation)
    );
  ...
  private async executeExpert(
    ...
    triggerUserId: string,
    expert: Expert,
    context: { documentContent?: string; chatHistory?: string },
    isIndirectInvocation: boolean
  ): Promise<void> {
    try {
      // Emit started event
      ...
      const response = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error(`Timeout after ${AI_TIMEOUT_MS}ms`)), AI_TIMEOUT_MS)
        )
      ]);
      ...
      if (response.toolCalls && response.toolCalls.length > 0) {
        const toolResults = await Promise.all(
          response.toolCalls.map(tc =>
            this.toolSystem.executeTool(tc.name, tc.arguments, {
              documentId,
              expertUserId: expert.userId,
              triggerUserId
            })
      ...
      if (this.loggerProvider) {
        await this.loggerProvider.logError(`expert_${expert.userId}`, error as Error);
      }
   ...
   */
  getModelDiagnostics(modelName: string) {
